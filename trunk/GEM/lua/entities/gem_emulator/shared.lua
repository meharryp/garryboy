...
function ENT:Enter()
	if CLIENT then
		if self.Emulator and self.Emulator:IsDebugging() then return end
		...
		elseif not self.Using then
			self.Using = true
			self.OldPos = self:GetPly():GetPos()
			--self:GetPly():SetPos( self:GetPos() + self:GetUp() * 80 - Vector(0,0,60) )
			local mul = (self:OBBMins()-self:OBBMaxs()):Length() / 1.7
			self:GetPly():SetPos( self:GetPos() + self:GetUp() * mul - Vector(0,0,60) )
...
function ENT:Exit()
	if CLIENT then
		...
		self:CreateLabel( "Game Paused" )
		self:CreateButton( "Resume", function( self ) self:ClearMenuItems() self:Enter() end, 68 )
		self:CreateButton( "Main Menu", function( self ) self:LoadMenu( "Main" ) end, 68 )	
	else
		if not self:GetPly() or not self:GetPly():IsValid() then
			self.Using = nil
		elseif self.Using then
			...
			ang.p = 0
			if self.OldPos then
				self:GetPly():SetPos( self.OldPos )
				self.OldPos = nil
			end
